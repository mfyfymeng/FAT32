...
static void set_diritem_cluster(diritem_t* item, u32_t cluster) {
	...
	item->DIR_FstClusHI = (u16_t)(cluster & 0xFFFF);
}
...
static xfat_err_t allocate_free_cluster(xfat_t* xfat, u32_t curr_cluster, u32_t count,
	...
	for (u32_t i = 2; (i < cluster_count) && (allocate_count < count); i++) {
	...
	if (allocate_count) {
		...
		if (en_erase) {
			...
			for (u32_t i = 0; i < allocate_count; i++) {
	...
	if (r_allocated_count)
		*r_allocated_count = allocate_count;
